...
import at.ac.uibk.core.functions.objects.*;
...
public class WorkflowConversionService {
     ...
     * @return
     */
    public static Workflow getAdaptedWorkflow(Workflow wf, Map<String, List> adaptations) {
        ...
        try {
            ...
            for (Map.Entry<String, List> e : adaptations.entrySet()) {
                ...
                if (fn instanceof ParallelFor) {
                    ...
                    List<Object> dataItemsUsed = getDataItemsBySource(parFor,
                            parentList.subList(parentList.indexOf(parFor), parentList.size()));
                    ...
                    for (int i = 0; i < numberOfDivides; i++) {
                        ...
                        Map<String, Object> loopCounterInfo = (Map) e.getValue().get(i);
    ...
    public static List<Function> getFunctionsWithNameBeginning(Workflow workflow, String nameLike) {
        final List<Function> foundFunctions = new LinkedList<>();
        traverseFunctions(workflow.getWorkflowBody(), (fn, parentObj) -> {
            if (fn.getName() != null && fn.getName().startsWith(nameLike)) {
